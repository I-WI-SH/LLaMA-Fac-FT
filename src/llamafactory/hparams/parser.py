...
def _verify_model_args(
    ...
    if model_args.adapter_name_or_path is not None and finetuning_args.finetuning_type not in ["lora", "adalora", "snndlft"]:
        raise ValueError("Adapter is only valid for the LoRA method.")

    if model_args.quantization_bit is not None:
        if finetuning_args.finetuning_type not in ["lora", "adalora", "snndlft"]:
...
def get_train_args(args: Optional[Dict[str, Any]] = None) -> _TRAIN_CLS:
    ...
    if (
        ...
        and finetuning_args.finetuning_type in ["lora", "adalora"]
