...
if TYPE_CHECKING:
    ...
    from ..hparams import FinetuningArguments, ModelArguments, QuantInitialArguments, EQFTConfig
...
def _setup_adalora_tuning(
    ...
    if is_trainable and latest_checkpoint is None: # create new lora weights while training
        ...
        if finetuning_args.finetuning_type == 'adalora':
            ...
            model = ada_get_peft_model(model, adalora_config)
...
def init_adapter(
    ...
    elif finetuning_args.finetuning_type == "adalora" or finetuning_args.finetuning_type == "snndlft" :
        model = _setup_adalora_tuning(
            config, model,model_args, finetuning_args, is_trainable, cast_trainable_params_to_fp32
        )
    elif finetuning_args.finetuning_type == "eqft":
        model = _setup_eqft_tuning(
            config, model, model_args, finetuning_args, is_trainable, cast_trainable_params_to_fp32
        )
    elif finetuning_args.finetuning_type == "meqft":
        model = _setup_meqft_tuning(
            config, model, model_args, finetuning_args, is_trainable, cast_trainable_params_to_fp32
        )
    else:
