...
@torch.no_grad()
def eqft_init(weight: Union[torch.Tensor, torch.nn.Parameter], num_bits: int, reduced_rank: int, num_iter=2):
    if is_bnb_available():
        import bitsandbytes as bnb
    else:
        raise ValueError("bitsandbytes is not available, please install it to use EQFT.")
    if num_bits not in [2, 3, 4]:
        raise ValueError("Only support 2, 3, 4 bits quantization")
    ...
    for i in range(num_iter):
        ...
        if num_bits == 4 and is_bnb_4bit_available():
            qweight = bnb.nn.Params4bit(
                residual.to("cpu"), requires_grad=False, compress_statistics=False, quant_type="nf4"
            ).to(compute_device)
