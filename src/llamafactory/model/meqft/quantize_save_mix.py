...
class EQFTConfig:
    ...
    eqft_ratio: int = field(default=0.5, metadata={"help": "Quantization bits for EQFT"})
...
def lora_initialize(lora_model, eqft_config, lora_config):
    from .meqft_utils import meqft_init
...
def quantize_and_save_mix(model_args, finetuning_args):
    ...
    if any(name in model_args.model_name_or_path.lower() for name in ["qwen"]):
    ...
    eqft_config = EQFTConfig(eqft_bits=finetuning_args.bits,
                             eqft_ratio=finetuning_args.ratio,
                             eqft_iter=finetuning_args.iter)

    lora_config = LoraConfig(
        task_type=task_type,
        inference_mode=True,
        r=finetuning_args.rank,
        lora_alpha=finetuning_args.rank,
        lora_dropout=0.1,
    ...
    model_name = model_args.model_name_or_path.split("/")[-1] + f"-{2 + finetuning_args.ratio:.2f}bit" + f"-{finetuning_args.rank}rank"
