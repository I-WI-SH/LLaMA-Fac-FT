...
from ...train.callbacks import PissaConvertCallback, SaveProcessorCallback
from ...train.trainer_utils import create_custom_optimizer, create_custom_scheduler
...
from ..snndlft import SNNPeftModel
...
def _is_peft_model(model):
    if is_peft_available():
        classes_to_check = (PeftModel, SNNPeftModel) if is_peft_available() else ()
...
class Trainer:
                    ...
                    if do_sync_step:
                        ...
                        if isinstance(model, SNNPeftModel):
                            if getattr(model.base_model, "update_and_allocate", None) is not None:
                                # import pdb
                                # pdb.set_trace()
                                model.base_model.update_and_allocate(total_batched_samples, self.finetuning_args.layer_wise)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        # import pdb
        # pdb.set_trace()
        supported_classes = (PreTrainedModel,) if not is_peft_available() else (PreTrainedModel, PeftModel, SNNPeftModel)
        # Save a trained model and configuration using `save_pretrained()`.
        # They can then be reloaded using `from_pretrained()`
