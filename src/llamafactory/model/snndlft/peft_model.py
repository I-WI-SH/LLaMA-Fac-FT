...
PEFT_TYPE_TO_MODEL_MAPPING = {
    ...
    PeftType.ADALORA: SNNAdaLoraModel,
...
class SNNPeftModel(PushToHubMixin, torch.nn.Module):
    """
...
class PeftModelForSequenceClassification(SNNPeftModel):
    """
...
class PeftModelForCausalLM(SNNPeftModel):
    """
...
class PeftModelForSeq2SeqLM(SNNPeftModel):
    """
...
class PeftModelForTokenClassification(SNNPeftModel):
    """
...
class PeftModelForQuestionAnswering(SNNPeftModel):
    """
...
class PeftModelForFeatureExtraction(SNNPeftModel):
    """
...
def get_layer_status(model: torch.nn.Module) -> list[TunerLayerStatus]:
    ...
    """
    if isinstance(model, SNNPeftModel):
...
def get_model_status(model: torch.nn.Module) -> TunerModelStatus:
    ...
    if isinstance(model, SNNPeftModel):
        ...
        trainable_params, total_params = SNNPeftModel.get_nb_trainable_parameters(model)
        base_model = model
        peft_types = {}
