...
from .layer import SNNAdaLoraLayer, DLRankAllocator, SNNSVDLinear


class SNNAdaLoraModel(LoraModel):
    """
    ...
    **Attributes**:
    ...
    def __init__(self, model, config, adapter_name):
        ...
        if self.peft_config[adapter_name].inference_mode:
            ...
            self.rankallocator = DLRankAllocator(self.model, self.peft_config[adapter_name], self.trainable_adapter_name)
    ...
    def _create_and_replace(
        ...
        if not isinstance(target, SNNAdaLoraLayer):
    ...
    def _create_new_module(lora_config, adapter_name, target, **kwargs):
        ...
        if loaded_in_8bit and isinstance(target_base_layer, bnb.nn.Linear8bitLt):
            ...
            new_module = SNNSVDLinear(target, adapter_name, **kwargs)
    ...
    def update_and_allocate(self, global_step, layer_wise):
        """
        ...
        if global_step < lora_config.total_step - lora_config.tfinal:
            _, rank_pattern = self.rankallocator.update_and_allocate(self.model, global_step, layer_wise)
            if rank_pattern:
                lora_config.rank_pattern = rank_pattern
        # Finalize the budget allocation
        elif global_step == lora_config.total_step - lora_config.tfinal:
            _, rank_pattern = self.rankallocator.update_and_allocate(self.model, global_step, layer_wise, force_mask=True)
